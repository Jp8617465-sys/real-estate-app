...
describe('PageMotion', () => {
  it('renders children', () => {
    render(<PageMotion><p>Content</p></PageMotion>);
    expect(screen.getByText('Content')).toBeInTheDocument();
  });

  it('renders a plain div when reduced motion is true', () => {
    vi.mocked(useReducedMotion).mockReturnValueOnce(true);
    const { container } = render(<PageMotion><p>No anim</p></PageMotion>);
    expect(container.firstChild?.nodeName).toBe('DIV');
  });

  it('passes className to wrapper', () => {
    vi.mocked(useReducedMotion).mockReturnValueOnce(true);
    const { container } = render(<PageMotion className="p-4"><p>x</p></PageMotion>);
...
describe('StaggerContainer', () => {
  it('renders children', () => {
    render(
      <StaggerContainer>
        <MotionItem><p>Item 1</p></MotionItem>
        <MotionItem><p>Item 2</p></MotionItem>
      </StaggerContainer>,
    );
  ...
  it('falls back to plain divs in reduced motion', () => {
    ...
    const { container } = render(
      <StaggerContainer>
        <MotionItem><span>x</span></MotionItem>
      </StaggerContainer>,
    );
