...
async function getOfficeId(supabase: ReturnType<typeof createSupabaseClient>, userId: string): Promise<string | null> {
...
export async function teamRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/team/members', async (request, reply) => {
    ...
    if (!officeId) return reply.status(400).send({ error: 'No office associated with this account' });
  ...
  fastify.patch<{ Params: { id: string } }>('/team/assignment-rules/:id', async (request, reply) => {
    const supabase = createSupabaseClient(request);
    const {
      data: { user },
      error: authError,
  ...
  fastify.delete<{ Params: { id: string } }>('/team/assignment-rules/:id', async (request, reply) => {
    const supabase = createSupabaseClient(request);
    const {
      data: { user },
      error: authError,
  ...
  fastify.post<{ Params: { id: string } }>('/team/workflow-templates/:id/share', async (request, reply) => {
    const supabase = createSupabaseClient(request);
    const {
      data: { user },
      error: authError,
  ...
  fastify.delete<{ Params: { id: string } }>('/team/workflow-templates/:id/share', async (request, reply) => {
    const supabase = createSupabaseClient(request);
    const {
      data: { user },
      error: authError,
