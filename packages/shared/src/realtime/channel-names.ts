...
export const RealtimeChannels = {
  ...
  pipeline: (pipelineType: string) => `realflow:pipeline:${pipelineType}` as const,
  ...
  conversationMessages: (contactId: string) => `realflow:messages:${contactId}` as const,

  /** Inbox-level message notifications (all inbound messages for an agent) */
  inboxMessages: (agentId: string) => `realflow:inbox:${agentId}` as const,

  /** Workflow execution status updates */
  workflowRuns: (workflowId: string) => `realflow:workflow-runs:${workflowId}` as const,

  /** All workflow runs (unscoped) */
  workflowRunsAll: () => 'realflow:workflow-runs:all' as const,

  /** Document changes for a contact */
  documents: (contactId: string) => `realflow:documents:${contactId}` as const,

  /** Notifications for a user */
  notifications: (userId: string) => `realflow:notifications:${userId}` as const,

  /** Portal search progress for a contact */
  searchProgress: (contactId: string) => `realflow:search-progress:${contactId}` as const,
} as const;
