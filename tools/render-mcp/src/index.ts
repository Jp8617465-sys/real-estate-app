...
function getApiKey(): string {
  ...
  if (!key) {
    ...
    process.stderr.write('[render-mcp] FATAL: RENDER_API_KEY environment variable is not set.\n');
    process.exit(1);
  }
  return key;
}

async function renderFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Render API error ${res.status} ${res.statusText} — ${url}\n${body}`);
  }
...
server.tool(
  ...
  ].join(' '),
  {
    ...
    cursor: z.string().optional().describe('Pagination cursor from a previous list_services call'),
    name: z.string().optional().describe('Filter services whose name contains this string'),
    type: z
  ...
  async ({ limit, cursor, name, type }) => {
    ...
    const items = await renderFetch<ListServicesItem[]>(`/services?${params.toString()}`);
...
server.tool(
  ...
  ].join(' '),
  {
    serviceId: z
      .string()
      .min(1)
      .describe('The Render service ID (e.g. srv-xxxxxxxxxxxxxxxxxx). Obtain from list_services.'),
  },
  async ({ serviceId }) => {
    const service = await renderFetch<RenderService>(`/services/${encodeURIComponent(serviceId)}`);
